...
def U_mag(u,v,w):
    umagsquare = np.square(u) + np.square(v) + np.square(w)
    umag = np.sqrt(umagsquare)
    return umag

def COMPUTE_CIRCULATION(U,V,x,y,xC,yC):
    
    fx = interpolate.RectBivariateSpline(y,x,U)
    fy = interpolate.RectBivariateSpline(y,x,V)
    uC = fx.ev(yC,xC)
    vC = fy.ev(yC,xC)
    Gamma = -(np.trapz(uC,xC)+np.trapz(vC,yC))
    return Gamma,uC,vC
